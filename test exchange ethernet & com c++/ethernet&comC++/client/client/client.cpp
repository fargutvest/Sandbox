...
int ExchangeEthernet()
{
	...
	while (true) {
		...
		int sendBytes = send(SocketExchange, byteSend, nMsgLen, 0);
		if ( sendBytes < nMsgLen) {
...
int ExchangeCOMport()
{
	...
	//do{
		char *bytesSend;
		printf("input data \n");
		bytesSend = "privet freund";	
		int len = strlen(bytesSend);
		int t = 32*456;
		int t1 = 32*939;
		int t2 = 32*1231;
		int t3 = 32*433;
...
WriteFile(hCom, bytesSend, len, &bWritten, NULL); //отправка
